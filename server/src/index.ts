...
export class App {
    ...
    async config(socketIO?: Server) {
        ...
        this.app.get('/api/v1/marketplaces/chatflows', async (req: Request, res: Response) => {
            ...
            const FlowiseDocsQnA = templates.find((tmp) => tmp.name === 'autoGPTChatflow')
            const FlowiseDocsQnAIndex = templates.findIndex((tmp) => tmp.name === 'autoGPTChatflow')
