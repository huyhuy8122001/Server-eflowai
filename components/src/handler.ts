...
import { AgentAction, BaseMessage, ChainValues } from 'langchain/schema'
import { Logger } from 'winston'
import { Server } from 'socket.io'
import openaiTokenCounter from 'openai-gpt-token-counter';
import { calculate_openai_cost } from './pricing';
...
export class ConsoleCallbackHandler extends BaseTracer {
    ...
    async onChainEnd(run: Run) {
        ...
        console.log('==============')
        console.log(run)
        console.log('======CHILD_RUN========')
        console.log(run.child_runs[0]);
        console.log('======CHILD_RUN_EXTRA========')
        console.log(run.child_runs[0].extra);
        console.log('==============')
        let messages: Message[] = run.inputs.chat_history.map((item: BaseMessage) => ({name: item.name, content: item.content}))
        console.log(messages);  
        const model = run.child_runs[0].extra?.invocation_params?.model || "gpt-3.5-turbo"; // Replace with your desired OpenAI chat model
        const tokenCount = openaiTokenCounter.chat(messages, model);
        console.log(`Token count: ${tokenCount}`);
        console.log(`Pricing: ${await calculate_openai_cost(model, tokenCount)}`)
    }
